...
export const QuandoLeboratorio = () => {
  ...
  const salvarTecnico = async (id, updatedOS) => {
	...
	const handleBuscarOrdensDeServico = async function() {
		try {
			const resultado = await OrdensDeServicoService.quadroLaboratorio();
			...
			notification.error({ message: `Erro ao buscar as ordens de servico (Laboratorio)` });
		}
	}
					...
					<Flex alignItems="center">
						<Flex flexDirection="row">
							...
							<Button
								...
								onClick={(ev) => {
									ev.stopPropagation();
									navigate(`/app/dashboards/editar-ordem-de-servico/${record.rawOrdem.id}?previous=/app/quadros/laboratorio`);
								}}
							/>
						</Flex>
					</Flex>
...
export default QuandoLeboratorio;
